            ...
            <div id="audit-tab" class="glass-card card" style="display: block;">
                ...
                <div style="overflow-x: auto; max-height: 60vh;">
                    <table class="responsive-table text-sm">
                        <thead><tr><th>時間</th><th>帳戶姓名</th><th>帳號</th><th>密碼</th><th>操作</th><th>詳細內容</th></tr></thead>
                        <tbody>
                            ${auditLogs.map(l => `
                                <tr>
                                    <td data-label="時間" class="text-muted" style="white-space:nowrap;">${new Date(l.timestamp).toLocaleString()}</td>
                                    <td data-label="帳戶姓名"><span class="badge badge-primary">${l.user || 'System'}</span></td>
                                    <td data-label="帳號"><code style="background:#eee; padding:2px 4px; border-radius:4px;">${l.account || '-'}</code></td>
                                    <td data-label="密碼" class="text-muted">${l.password || '-'}</td>
                                    <td data-label="操作" class="font-bold">${l.action}</td>
                                    <td data-label="詳細內容" class="text-muted">${l.details || '-'}</td>
                                </tr>
                            `).join('') || '<tr><td colspan="6" class="text-center p-4">目前無紀錄</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>
