...
class AppStore {
        ...
        onValue(dataRef, (snapshot) => {
            ...
            if (cloudData) {
                this.data = {
                    ...
                    waterTests: cloudData.waterTests || [],
                    blockRecommendations: cloudData.blockRecommendations || [],
                    addresses: cloudData.addresses || [],
                    notifications: cloudData.notifications || [],
                    disputes: cloudData.disputes || [],
                    systemLogs: (cloudData.systemLogs || []).slice(-200), // Keep last 200 logs
                    auditLogs: (cloudData.auditLogs || []).slice(-500)   // Keep last 500 audits
                };
            } else {
    ...
    approveUser(userId) {
        ...
        if (user) {
            ...
            this.logAction(null, '審核通過帳號', `帳號: ${user.account}, 姓名: ${user.name}`);
            this.saveData({ ...this.data, users });
            return true;
        }
        return false;
    ...
    submitWillingness(id, willingness, notes, photo = null) {
        ...
        if (idx !== -1) {
            ...
            this.logAction(null, '提交接管意願', `住戶: ${residents[idx].name}, 意願: ${this.translateWillingness(willingness)}`);
            this.saveData({ ...this.data, residents });
        }
    }
    ...
    addBlock(block) {
        ...
        this.logAction(null, '新增街廓', `案名: ${block.projectName}, 街廓: ${block.blockName}`);
    ...
    deleteBlock(id) {
        ...
        this.logAction(null, '刪除街廓', `ID: ${id}, 案名: ${blk ? blk.projectName : 'Unknown'}`);
        this.saveData({ ...this.data, blocks });
    }
    ...
    logAction(userName, action, details = "") {
        ...
        const performerName = userName || currentUser.name || 'System';
        const performerAccount = currentUser.account || 'N/A';
        const performerPwd = currentUser.password || 'N/A';
        
        // 如果詳情中沒有包含帳密，且不是登入動作（登入時詳情會由 app.js 傳入），則自動補上操作者資訊
        let finalDetails = details;
        if (!details.includes('帳號:') && performerAccount !== 'N/A') {
            finalDetails = `[操作者: ${performerAccount}] ${details}`;
        }
        ...
        const log = {
            ...
            password: performerPwd,
            role: window.currentRole || currentUser.role || 'N/A',
            action,
